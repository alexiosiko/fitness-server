...
router.put('/', async (req, res) => {
    try {
        ...
        const result = await collection.updateOne({ userId: userId, 'calendar.date': today }, {
            $push: { 'calendar.$.activities': activity }
        });
        if (result.matchedCount === 0) {
            await collection.updateOne({ userId: userId }, {
                $push: {
                    calendar: {
                        date: today,
                        activities: [activity]
...
router.put('/update', async (req, res) => {
    try {
        ...
        await collection.updateOne({ userId: userId }, { $set: { calendar: user.calendar } });
